...
# python accuracy_top_k.py --model_arch "resnet34" --gpu 6 --box_preset 0 --order 1
...
import numpy as np
import torch
from sklearn.metrics import confusion_matrix, accuracy_score, balanced_accuracy_score, top_k_accuracy_score
...
def inference(cfg, best_model_checkpoint, acc_order):
    ...
    with torch.no_grad():
        ...
        print("Accuracy (Top 1): ", acc, "Balanced Accuracy:", acc_bal)
        
        acc_ks = []
        for k in range(1, 20):
            acc_k = top_k_accuracy_score(gt, all_results[:, :679], k=k)
            acc_ks.append(acc_k)

        print(f"Accuracy (Top {acc_order}): ", acc_k)
        ...
        return acc_ks, gt, all_results[:, :679]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--acc_order", type=int, default=1, choices={1,2,3,4,5,6},
                        help="Select the order of the accuracy calculation ")
    ...
    if best_model is not None:
        inference(cfg=cfg, 
        best_model_checkpoint=best_model, 
        acc_order=args.acc_order)
