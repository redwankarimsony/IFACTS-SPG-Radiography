...
class AgentSiameseNetwork:
    def __init__(self, config):
        ...
        self.device = torch.device(self.config['cuda_device'])
        ...
        self.net = SiameseNetwork(network=self.network, in_channels=self.n_channels, n_features=self.n_features).cuda(device=self.device)
        self.net = nn.DataParallel(self.net)
        self.best_net = SiameseNetwork(network=self.network, in_channels=self.n_channels, n_features=self.n_features).cuda(device=self.device)

        # Choose loss function
        if self.loss_method == 'BCEWithLogitsLoss':
            self.loss = nn.BCEWithLogitsLoss().cuda(device=self.device)
        else:
        ...
        if self.data_handling != 'randomized':
    ...
    def training_validation(self):
        ...
        for epoch in range(self.start_epoch, self.max_epochs):
            ...
            training_loss = Utils.train(self.net, self.training_loader, self.n_samples_train, self.batch_size,
                                        self.loss, self.optimizer, epoch, self.max_epochs, self.device)
            validation_loss = Utils.validate(self.net, self.validation_loader, self.n_samples_val, self.batch_size,
                                             self.loss, epoch, self.max_epochs, self.device)
    ...
    def testing_evaluation(self):
        # Testing phase!
        y_true, y_pred = Utils.test(self.best_net, self.test_loader, self.device)
