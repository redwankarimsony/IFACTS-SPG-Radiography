...
def applyCLAHE(image, display=False):
...
def applyHistogramEqualization(image, display=False):
    ...
    if display:
        ...
        # Display the original image
        fig.add_subplot(rows, cols, 1)
        plt.imshow(image, cmap=plt.cm.gray); plt.axis('off'); plt.title("Input Image")
...
def applyHFEFilter(image, display=False):
    #https://123machinelearn.wordpress.com/2017/12/25/image-enhancement-using-high-frequency-emphasis-filtering-and-histogram-equalization/
    image_bw = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
